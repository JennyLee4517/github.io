...
[문제보기](https://www.acmicpc.net/problem/10972)

```java
import java.util.*;
public class Main
{
  // 다음 순열 이란 ? 사전순으로 다음 번째가 되는 순열을 말한다
  public static boolean next_perm(int[] arr){ // 다음 순열로 변경시켜줌 & 다음 순열의 존재여부 반환
    int i = arr.length - 1; // 끝부터 탐색한다

    while( i > 0 && arr[i-1] >= arr[i]){ // 앞쪽으로 쭉 탐색하면서 언제부터 내림차순이 안먹히는지 찾는다 (이전순열의 경우 부등호 체인지)
      i--;
    }
    ...
    while(arr[j] <= arr[i-1] ){ // arr[i-1] 보다 크면서 & 뒷파트에서 가장 꼬붕인 애를 찾는것임 (이전순열의 경우 부등호 체인지)
      j--;
    }
